...
class TestDeleteNotification(unittest.TestCase):
    ...
    def test_delete_notification_by_id(self):
        with self.application.app_context():
            ...
            response = self.client.delete(f"{self.endpoint}{self.notif['id']}")
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json(), {"message": "Success"})
            self.assertIsNone(db.session.query(Notifications).filter_by(id=self.notif["id"]).first())
    
    def test_delete_notification_by_id_not_found(self):
        with self.application.app_context():
            self.client.cookies = {"session_id": self.session_id}
